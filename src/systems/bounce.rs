    ...
    fn run(&mut self, (mut balls, paddles, transforms): Self::SystemData) {
        ...
        for (ball, transform) in (&mut balls, &transforms).join() {
            ...
            for (paddle, paddle_transform) in (&paddles, &transforms).join() {
                ...
                // To determine wether the ball has collided with a paddle, we create a larger
                // rectangle around the current one, by subtracting the ball radius form the 
                // lowest coordinates, and adding the ball radius to the highest ones. 
                // The ball is then within the paddles if its centre is within the larger wrapper
                // rectangle.
                if point_in_rect(
                    ball_x,
                    ball_y,
